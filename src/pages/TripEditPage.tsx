...
export default function TripEditPage() {
  ...
  const addDay = () => setLocalTrip((p) => { if (!p) return p; const last = p.days[p.days.length - 1]; const nd = last ? format(new Date(new Date(last.date).getTime() + 86400000), 'yyyy-MM-dd') : p.startDate; return { ...p, days: [...p.days, { dayNumber: p.days.length + 1, date: nd, title: `День ${p.days.length + 1}`, activities: [] }], updatedAt: new Date().toISOString() }; });
  ...
  const addAct = (i: number) => updDay(i, (d) => ({ ...d, activities: [...d.activities, { id: uuidv4(), time: '09:00', title: 'Новая активность', description: '', location: '', notes: '' }] }));
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="space-y-4">
        {localTrip.days.map((day, i) => (
          <DayCard key={day.dayNumber} day={day} dayIndex={i} totalDays={localTrip.days.length}
